        ...
        <div class="flex mt-[80px]">        
            <div class="bg-gray-800 text-white p-4 fixed h-full w-64 overflow-y-auto pt-40">        
                <div class="flex items-center space-x-4 mb">      
                    <div class="">      
                        <img src="images/default.png" alt="User" class="w-20 h-20 rounded-full mb-5 mx-16">      
                        <%
                            User user = (User) request.getSession().getAttribute("user"); // Ambil user dari session      
                            if (user != null) {
                        %>      
                        <div class="font-bold mx-20 "><%= user.getName()%></div>      
                        <div class="text-sm text-gray-400 mx-20 mb-2"><%= user.getRole()%></div>      
                ...
                <div class="space-y-4 pt-8">        
                    ...
                    <div class="mt-8">        
                        ...
                        <div id="settings-menu" class="hidden pl-4 space-y-2"> 
                            <button class="w-full text-left text-gray-200 hover:bg-gray-700 p-2 rounded" onclick="window.location.href = 'Profile.jsp'">          
                                <i class="fas fa-user-edit align-middle"></i> Edit Profile          
                            </button> 
        ...
        <script>
            function loadData(menu) {
                ...
                if (menu === 'buku') {
                    ...
                    fetch('Buku?menu=card') // Fetch data from Buku controller for books      
                            .then(response => {
                                if (!response.ok) {
                    ...
                    titleElement.innerText = 'Keranjang Pinjaman';
                    fetch('Keranjang.jsp') // Fetch data from Keranjang.jsp      
                            .then(response => {
                                if (!response.ok) {
                                    throw new Error('Network response was not ok');
                                }
                    ...
                    fetch('Majalah?menu=card') // Fetch data from UserItem controller for magazines      
                            .then(response => {
                                if (!response.ok) {
                                    throw new Error('Network response was not ok');
                                }
                    ...
                    fetch('Jurnal?menu=card') // Fetch data from UserItem controller for journals      
                            .then(response => {
                                if (!response.ok) {
                                    throw new Error('Network response was not ok');
                                }
